...
**MacPicker.Pro** - 一个智能 Mac 选购助手，帮助用户 3 步快速定位目标 Mac：

```
第一步：锁预算（快捷按钮）→ 第二步：定配置（RAM/SSD）→ 第三步：找机型（搜索下拉）
```
...
| 功能 | 效果 |
|------|------|
| **快捷预算按钮** | 点击 [8k-12k] 一键选择价格区间 |
| **搜索下拉** | 输入 M → 立即显示 M1、M2、MacBook... |
| **已选条件标签** | 显示当前所有筛选，点 × 快速删除 |
| **实时反馈** | PC 端 300ms 自动更新，无需点"应用" |
| **容错机制** | 自动交换错误的价格顺序，避免误操作 |

### 📖 核心特性说明

打开 `pages/_app.js`，你会看到 LiquidBackground 配置：
